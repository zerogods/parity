...
impl Miner {
	...
	fn prepare_block(&self, chain: &MiningBlockChainClient) -> (ClosedBlock, Option<H256>) {
		...
		let (transactions, mut open_block, original_work_hash) = {
			let max_per_sender = if chain_info.best_block_number + 1 >= self.engine.params().dust_protection_transition {
				Some(self.engine.params().max_txs_per_account_per_block)
			} else { None };
			let transactions = {self.transaction_queue.lock().top_transactions_at(chain_info.best_block_number, chain_info.best_block_timestamp, max_per_sender)};
